...
- **등록 tool 이름 33 개** (8 카테고리: primary 28 개 + compatibility alias 5 개)
...
### OpenAPI 캐시 (`openapi_*`, legacy `swagger_*` alias)
...
Compatibility alias: `swagger_get`, `swagger_refresh`, `swagger_status`, `swagger_search`, `swagger_envs` 는 기존 workflow 용으로 계속 등록하지만, 새 문서/스킬은 `openapi_*` 를 우선 사용한다.
...
- **Conducted by**: `rocky`.
- **사용 도구**: `openapi_get`, `openapi_status`, `openapi_refresh`, `openapi_search`, `openapi_envs`. 기존 `swagger_*` 이름은 compatibility alias 로 유지.
