...
- **도구 29 개** (9 카테고리)
- **스킬 8 개** (`notion-context`, `openapi-client`, `mysql-query`, `spec-pact`, `pr-review-watch`, `spec-to-issues`, `gh-passthrough`, `seo-validation`)
...
`ogpeek` 라이브러리 (npm `ogpeek@^0.5.0`) 를 wrapping 한 단일 URL OG / Twitter Card / JSON-LD / favicon 메타 검증.

#### `seo_validate`

- **What**: http(s) URL 한 개를 fetch 후 `ogpeek.parse` 로 분석. `summary` (finalUrl / redirects / og:title / og:description / og:image / og:type / og:url / canonical / errors / warnings / info / hasJsonLd / hasFavicon / iconCount) 와 `raw: OgDebugResult` 둘 다 반환. 기본 SSRF 가드는 private / loopback / link-local / IPv6 ULA 호스트 차단.
- **Input**: `url: string` (필수, http/https 만), `timeoutMs?: number` (1..30000, 기본 8000), `allowPrivateHosts?: boolean` (기본 `seo.allowPrivateHosts ?? false`).
- **Output**: `{ summary: SeoValidateSummary, raw: OgDebugResult }`. `summary.errors` / `warnings` / `info` 는 `raw.warnings` 를 severity 별 (`error` / `warn` / `info`) 로 분리한 것. ogpeek 의 13 종 warning code 를 그대로 surface — `OG_TITLE_MISSING`, `OG_TITLE_TOO_LONG`, `OG_TYPE_MISSING`, `OG_IMAGE_MISSING`, `OG_URL_MISSING`, `OG_URL_MISMATCH`, `OG_TYPE_UNKNOWN`, `URL_NOT_ABSOLUTE`, `DUPLICATE_SINGLETON`, `ORPHAN_STRUCTURED_PROPERTY`, `INVALID_DIMENSION`, `MISSING_PREFIX_ATTR`, `JSONLD_PARSE_ERROR`.
- **Owner**: `seo-validation` 스킬 (rocky 가 conduct).
- **Side effects**: 호출당 1 회 outbound HTTP 요청 (redirect ≤ 5, body ≤ 5 MiB — ogpeek 기본). 저널은 사용자가 명시 요청할 때만 append.
- **Related config**: `seo.allowPrivateHosts` (기본 `false`), `seo.timeoutMs` (기본 `8000`). 도구 호출 인자가 config 보다 우선.
...
- **Conducted by**: `rocky`.
- **사용 도구**: `seo_validate`, `journal_append`.
- **목적**: 단일 http(s) URL 의 Open Graph / Twitter Card / JSON-LD / favicon 메타를 검증해 한국어 markdown 리포트 (표 + 에러 / 경고 / 권고) 로 surface. 한 턴에 한 URL — 다중 URL / sitemap 요청은 v1 범위 밖.
- **SSRF 정책**: 기본 ON. 사용자가 사내망 / localhost 검증을 명시할 때만 `allowPrivateHosts: true` 로 호출 또는 `agent-toolkit.json` 의 `seo.allowPrivateHosts: true` 로 설정.
...
| `seo.allowPrivateHosts` / `seo.timeoutMs` | `seo_validate` 기본값 | `boolean` / `integer` | `allowPrivateHosts` 기본 `false` — private / loopback / link-local / IPv6 ULA 호스트 차단. `timeoutMs` 기본 `8000` (1..30000). 도구 호출 인자가 config 보다 우선. |
