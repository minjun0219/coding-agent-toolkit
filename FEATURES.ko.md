...
- **Input**: `input` — Notion page id 또는 페이지 URL.
- **Output**: `notion_get` 과 같은 shape, `hit: false`.
...
- **What**: 단일 페이지 캐시 메타 (`cachedAt`, `ttlSeconds`, `expired`) 만 조회. remote 호출 없음.
- **Input**: `input` — Notion page id 또는 페이지 URL.
...
- **Input**: `input` — `swagger_get` 과 동일.
- **Output**: `swagger_get` 과 같은 shape, `hit: false`.
- **Owner**: `openapi-client` 스킬.
- **Side effects**: 두 캐시 파일 재작성.
- **Related config**: `swagger_get` 와 동일.

#### `swagger_status`

- **What**: 단일 spec 의 캐시 메타 (`cachedAt`, `ttlSeconds`, `expired`, `title`, `endpointCount`) 만 조회. 네트워크 호출 없음.
- **Input**: `input` — `swagger_get` 과 동일.
...
- **What**: `agent-toolkit.json` 의 `openapi.registry` 트리를 `[{ host, env, spec, url }]` 로 평면화. 네트워크 호출 없음.
- **Input**: 없음.
...
- **Output**: append 된 entry `{ id, timestamp, kind, content, tags, pageId? }`.
...
- **What**: `mysql.connections` 트리를 `{ handle, host, env, db, authMode, authEnv, hostName, port, user, database }` 로 평면화. **자격증명 *값* 은 노출하지 않고 env 변수 이름만 보여 준다.**
- **Input**: 없음.
- **Output**: `{ entries: [...] }`.
...
- **Output**: `{ rows, columns }` (경로에 따라 shape 다름).
- **Owner**: `mysql-query` 스킬.
...
- **Input**: `handle: string` (`owner/repo#NUMBER` 또는 github.com PR URL), `note?: string`, `labels?: string[]`, `mergeMode?: "merge" | "squash" | "rebase"`.
...
- **What**: 등록된 모든 active watch + 그 PR 들의 미처리 이벤트 합계.
- **Input**: 없음.
...
- **Output**: `issue_create_from_spec` 와 같은 plan shape (`applied` 는 항상 없음).
...
- **What**: `gh` subcommand 부터 시작하는 `args` 배열을 받아 `read` / `write` / `deny` 로 분류 후 정책 적용.
  ...
  - **write** (`issue create` / `label create` / `api --method POST` / …) — `dryRun: true` (기본) 면 plan 만, `dryRun: false` 로 명시해야 실행.
...
- **Conducted by**: `rocky`.
- **사용 도구**: `swagger_get`, `swagger_status`, `swagger_refresh`, `swagger_search`, `swagger_envs`.
...
- **Conducted by**: `mindy` (`pr_event_resolved` 단일 권한자).
...
- **라우팅**:
  ...
  - PR review watch 키워드 ("PR review" / "리뷰 봐줘" / "코멘트 확인" / "머지까지 watch" / "리뷰 답글" / "PR drift") → `@mindy`.
