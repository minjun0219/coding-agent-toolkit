...
| `github.repositories` | `owner/repo` allow-list for PR review watch | `{ alias?, labels?, defaultBranch?, mergeMode? }` | Token / secret leaves are rejected — auth lives with the external GitHub MCP. `mergeMode ∈ {"merge", "squash", "rebase"}`. |
