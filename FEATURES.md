...
| 변수 | 기본값 | 영향 |
| --- | --- | --- |
| `AGENT_TOOLKIT_OPENAPI_CACHE_DIR` | XDG cache (`$XDG_CACHE_HOME/openapi-mcp` 또는 `~/.cache/openapi-mcp`) | OpenAPI spec 디스크 캐시 위치. 세 host 모두 동일 변수 사용. |
| `AGENT_TOOLKIT_OPENAPI_CACHE_TTL` | `300` (초) | OpenAPI 캐시 TTL. fresh / stale 경계. |
| `AGENT_TOOLKIT_CONFIG` | `~/.config/opencode/agent-toolkit/agent-toolkit.json` | user-level `agent-toolkit.json` 경로 override (두 plugin 만). |

CLI 모드는 위 변수에 추가로 `openapi-mcp` 의 CLI flag (`--config`, `--log-level`, `--insecure-tls`) 를 사용한다.
