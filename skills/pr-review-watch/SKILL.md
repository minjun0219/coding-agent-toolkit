...
4. **Idempotency lives in the reducer.** `pr_event_record` returns `alreadySeen: true` when the same `(type, externalId)` already has an inbound entry — mindy uses this to skip already-processed comments without disk dedup.
...
- **`pr_event_resolve` 가 가리키는 inbound 가 없는 경우** → caller 가 잘못된 toolkitKey 를 줬다는 신호. pending 목록을 다시 surface 하고 멈춘다.
