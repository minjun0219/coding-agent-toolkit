...
external GitHub MCP                           toolkit (this skill / mindy)
  ├── pull_request_read           ─────►       PULL: unstable_pr_event_record (×N)
  ├── add_reply_to_pull_request_comment        VALIDATE: unstable_pr_event_resolve
  ├── add_issue_comment                        VALIDATE: unstable_pr_event_resolve
  ├── add_comment_to_pending_review            VALIDATE: unstable_pr_event_resolve
  ├── pull_request_review_write                VALIDATE: unstable_pr_event_resolve
...
1. **Verify the watch is active.** `unstable_pr_watch_status` — when the handle is not active, prompt the user to call WATCH-START first; do not silently start one.
...
4. **For each new event**, call `unstable_pr_event_record` with the appropriate type:
...
5. **List pending events** — `unstable_pr_event_pending` after recording finishes — and surface them as a numbered list to the user.
6. **Auto-stop on merge / close.** When a `merge` or `close` event was just recorded *and* the latest external MCP read confirms the PR is no longer open, transition automatically to **WATCH-STOP** by calling `unstable_pr_watch_stop` (`reason: merged` or `reason: closed`). This is the *only* automatic mode transition in the skill.
...
(머지/닫힘이 감지된 경우)
- `unstable_pr_watch_stop` 자동 호출됨 (reason: merged|closed). watch 종료.
```
...
1. **Pick the targets.** Either a specific 1-based index from the latest PULL list, or "모두" (= all currently pending). Resolve to `(handle, type, externalId)` triples by calling `unstable_pr_event_pending` and matching by `toolkitKey`.
2. **For each target**:
   ...
   5. **Resolve.** `unstable_pr_event_resolve` with `decision`, `reasoning` (한 줄), and `replyExternalId` (the comment id the MCP returned).
...
1. **Verify state.** `unstable_pr_watch_status` — when the handle is not active, return "이미 stop 상태" on a single line.
2. **Determine the reason.** `merged` (PR merged externally) / `closed` (closed without merge) / `manual` (user explicitly asked to stop) — `STOP_REASONS` enum 외 값은 `unstable_pr_watch_stop` 도구가 throw 한다 (자유 문자열 금지). When PULL detected merge / close in the same turn, that turn already auto-stopped — caller would only invoke WATCH-STOP manually for `manual`.
3. **Call `unstable_pr_watch_stop`** with `reason`.
4. **Surface a one-line confirmation** + a final pending count from `unstable_pr_event_pending` (so the user knows whether anything was left unresolved).
...
- **`unstable_pr_event_record` 가 `alreadySeen: true` 만 반환** → "새 이벤트 없음" 으로 응답하고 멈춘다 (PULL 결과 0 건).
- **`unstable_pr_event_resolve` 가 가리키는 inbound 가 없는 경우** → 도구 자체가 throw 한다 (`no prior pr_event_inbound for handle "..." + toolkitKey "..."`). orphan resolve 가 박히면 reducer 의 `resolvedKeys` 에 그 toolkitKey 가 포함되어 이후 진짜 inbound 가 들어와도 영구 제외 (큐 유실) 되므로 handler 단에서 끊는다. caller 는 `unstable_pr_event_pending` 으로 정확한 toolkitKey 를 다시 확인 후 재호출.
