...
- `lib/github-issue-sync.ts` — locked SPEC body parser + GitHub REST client (`fetch` only, no octokit) + idempotent epic/sub-issue planner. Marker (`<!-- spec-pact:slug=…:kind=… -->`) + label (`spec-pact`) dedupe; `dryRun` for plan-only.
...
- `skills/spec-to-issues/SKILL.md` — locked SPEC → GitHub Issue series (epic + N subs). Mapping fixed (1 SPEC = 1 epic, `# 합의 TODO` bullet 1 개 = 1 sub-issue). Idempotent on re-run. Conducted by `agents/rocky.md` after grace finalize/amend.
- `agents/rocky.md` — work-partner primary agent (`mode: all`) that conducts the toolkit's `notion-context` + `openapi-client` + `spec-to-issues` flows and the journal for users (and for any external primary agent that happens to share the environment, e.g. OmO Sisyphus or Superpowers — synergy when present, not a dependency), routes the SPEC-합의 lifecycle to `@grace`, and may delegate to external sub-agents / skills when a task exceeds the toolkit. Frontend specialty, fullstack range.
...
Only `AGENT_TOOLKIT_NOTION_MCP_URL` is required. The `spec-to-issues` skill additionally requires `AGENT_TOOLKIT_GITHUB_TOKEN` (PAT with `repo` scope, or fine-grained `Issues: Read & Write`) and either `agent-toolkit.json` `github.repo` or `AGENT_TOOLKIT_GITHUB_REPO` for the target repository. See the README env-var table for the rest.
...
5. If the plugin's tool contract changes, update the tool-usage rules in the relevant skill (`skills/notion-context/SKILL.md` for `notion_*`, `skills/openapi-client/SKILL.md` for `swagger_*`, `skills/spec-pact/SKILL.md` for the lifecycle modes that touch `notion_*` / `journal_*` / file IO, `skills/spec-to-issues/SKILL.md` for `issue_*`) **and** the corresponding routing / tool rules in `agents/rocky.md` (Rocky conducts the cache-first skills + `spec-to-issues` + journal and routes the lifecycle, so changes on either side propagate to it; `journal_*` is owned by `rocky` directly — no separate skill) **and** `agents/grace.md` (Grace conducts `spec-pact` end-to-end and is the single finalize/lock authority over `.agent/specs/INDEX.md` + SPEC files)
6. If `agent-toolkit.json` shape changes, update **both** `agent-toolkit.schema.json` (IDE autocomplete) **and** `lib/toolkit-config.ts` (runtime validation) — they must stay in lockstep. Currently the `openapi.registry`, the `spec` (dir / scanDirectorySpec / indexFile), and the `github` (repo / apiBaseUrl / defaultLabels) objects are the three top-level keys with explicit validation
