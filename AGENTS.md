...
- `.opencode/plugins/agent-toolkit.ts` — plugin entrypoint. `config` hook registers `skills/` and `agents/`, and exposes nineteen tools: `notion_get` / `notion_refresh` / `notion_status` / `notion_extract` + `swagger_get` / `swagger_refresh` / `swagger_status` / `swagger_search` / `swagger_envs` + `journal_append` / `journal_read` / `journal_search` / `journal_status` + `mysql_envs` / `mysql_status` / `mysql_tables` / `mysql_schema` / `mysql_query` + `spec_pact_fragment` (Phase 6.A — returns the matching `spec-pact` fragment body from the plugin's absolute install path).
