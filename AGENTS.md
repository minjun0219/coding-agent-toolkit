...
Dual-host MCP / plugin toolkit: **Claude Code is the primary host** (17-tool surface via `server/index.ts` registered through `.claude-plugin/plugin.json` + `.mcp.json`), **opencode is the secondary host** (27-tool surface via `.opencode/plugins/agent-toolkit-server.ts`). A **third standalone host** (`openapi-mcp` CLI) exposes only the 7 `openapi_*` tools for users who don't need the rest of the toolkit. All hosts share `lib/openapi/` and the `skills/` / `agents/` trees.

- **Claude Code surface (17 tools, primary)**: `openapi_*` (7) + `journal_*` (4) + `mysql_*` (5) + `spec_pact_fragment` (1).
- **opencode-only (10 tools, removal candidates)**: `notion_*` (4) + `pr_*` (6). Tracked under *MVP scope → Removal candidates* below.
- **`openapi-mcp` standalone (7 tools)**: `openapi_get` / `openapi_refresh` / `openapi_status` / `openapi_search` / `openapi_envs` / `openapi_endpoint` / `openapi_tags`. Same 7 tools the other hosts expose, but on top of an `openapi-mcp.json` config (`specs.<name>.environments.<env>.baseUrl`) instead of `agent-toolkit.json`'s `openapi.registry`. Entry: `bin/openapi-mcp` → `server/openapi-mcp/index.ts`.
...
- `server/index.ts` — **Claude Code MCP entrypoint** (primary). Stdio MCP server (`bun run server/index.ts`) registered by `.claude-plugin/plugin.json` + `.mcp.json`. Imports handlers from `.opencode/plugins/agent-toolkit.ts` (the opencode implementation file) and registers 17 tools: `openapi_*` (7) + `journal_*` (4) + `mysql_*` (5) + `spec_pact_fragment` (1). The other 10 tools (`notion_*` ×4, `pr_*` ×6) are intentionally omitted — see *MVP scope → Removal candidates*.
- `server/openapi-mcp/index.ts` — **standalone `openapi-mcp` MCP entrypoint** (third host). Stdio MCP server (`bin/openapi-mcp`) that wraps `lib/openapi/` only — no mysql / journal / pr / spec-pact / notion deps. Receives an `OpenApiMcpConfig` (`specs.<name>.environments.<env>.baseUrl`) and registers the same 7 `openapi_*` tools but with a slightly different input shape (spec name + environment, instead of URL or `host:env:spec` handle).
- `bin/openapi-mcp` — **`#!/usr/bin/env bun` shebang for the standalone entry**. Hand-rolled `--config` / `--log-level` / `--insecure-tls` / `-V` / `-h` parser (no `commander` dep) → `loadConfig` → `startStdioServer`. Exposed as `openapi-mcp` via `package.json` `bin`.
...
- `lib/openapi/` — **OpenAPI core** (v0.2 부터; absorbed from the now-archived `openapi-mcp-server`). Modular pieces shared by all three hosts: `cache.ts` (sha1-keyed disk cache with `schemaVersion: 1`), `fetcher.ts` (Bun `fetch`-based HTTP + conditional GET; root spec 과 외부 `$ref` 모두 동일 fetcher 경로 — `tls.rejectUnauthorized` / `tls.ca` 옵션 표준 지원), `parser.ts` (yaml + swagger2→3 conversion + `$ref` deref via `swagger-parser`), `indexer.ts` (`IndexedSpec` / `EndpointDetail` / `buildEndpointDetail`), `filter.ts` (scored keyword search: operationId>path>summary>description), `registry.ts` (memory + disk registry with TTL + background revalidation; `registerSpec` for runtime ad-hoc URLs), `schema.ts` (zod schema for `openapi-mcp.json`), `config-loader.ts` (XDG-respecting JSON / YAML loader), `adapter.ts` (`agent-toolkit.json` registry → `OpenApiMcpConfig` flattening, ephemeral URL helpers, `createAgentToolkitRegistry` factory), `logger.ts` (pino → stderr only). Shape validation lives one layer above in `lib/toolkit-config.ts` for the agent-toolkit registry, and in `lib/openapi/schema.ts` for the standalone config.
- `lib/openapi-registry.ts` — `host:env:spec` handle parsing, scope resolution, and registry flattening on top of the toolkit config. Reads the leaf via the helpers (`getRegistryUrl` / `getRegistryBaseUrl` / `getRegistryFormat`) so the leaf can be a string (legacy URL-only) or `{ url, baseUrl?, format? }` object.
...
- **Dependencies**: avoid adding any if possible. Prefer the standard library and Bun built-ins. **Explicit prod-dep exceptions:** (1) `mysql2` — MySQL has no native Bun client and the wire protocol / TLS / auth-plugin handling is too large to hand-roll for a "read-only inspection" surface; (2) `@modelcontextprotocol/sdk` + `zod` — required by `server/index.ts` (Claude Code MCP entrypoint). The MCP wire protocol (JSON-RPC framing, capability negotiation, content-block shape) is too large to hand-roll for the Claude Code surface; zod ships as the SDK's blessed schema dialect; (3) `@apidevtools/swagger-parser` + `swagger2openapi` + `js-yaml` + `openapi-types` + `pino` — added in v0.2 for `lib/openapi/` (absorbed from `openapi-mcp-server`). $ref dereference, swagger 2.0 → 3.0 conversion, YAML parsing, and stderr-only structured logging are too large / subtle to hand-roll. HTTP transport는 Bun의 native `fetch` (with `tls` option) 를 직접 쓰므로 별도 HTTP client (e.g. undici) 는 필요하지 않다. Dev-only tooling dependencies such as linters / formatters are allowed when explicitly agreed for repository workflow. New runtime deps beyond these require a separate scope discussion.
