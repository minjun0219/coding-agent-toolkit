...
- **Claude Code surface (15 tools, primary)**: `swagger_*` (5) + `journal_*` (4) + `mysql_*` (5) + `spec_pact_fragment` (1).
- **opencode-only (13 tools, removal candidates)**: `notion_*` (4) + `pr_*` (6) + `gh_run` (1) + `issue_*` (2). Tracked under *MVP scope → Removal candidates* below.
- **Skills (7)**: `notion-context`, `openapi-client`, `mysql-query`, `spec-pact`, `pr-review-watch`, `spec-to-issues`, `gh-passthrough`.
...
- **Handles & config**: OpenAPI specs / MySQL connections addressable by URL / fields or by `host:env:spec` / `host:env:db` handles in `agent-toolkit.json` (project `./.opencode/agent-toolkit.json` overrides user `~/.config/opencode/agent-toolkit/agent-toolkit.json`). GitHub repositories declared as `owner/repo` keys under `github.repositories` (no tokens — PR-watch credentials live in the external GitHub MCP, spec-to-issues credentials live with `gh auth login`). SPEC files at `.agent/specs/<slug>.md` (default) or `**/SPEC.md` (directory-scoped), discovered through `.agent/specs/INDEX.md`.
...
- `server/index.ts` — **Claude Code MCP entrypoint** (primary). Stdio MCP server (`bun run server/index.ts`) registered by `.claude-plugin/plugin.json` + `.mcp.json`. Exposes 15 tools: `swagger_*` (5) + `journal_*` (4) + `mysql_*` (5) + `spec_pact_fragment` (1). The other 13 tools (`notion_*` ×4, `pr_*` ×6, `gh_run`, `issue_*` ×2) are intentionally omitted — see *MVP scope → Removal candidates*.
- `.opencode/plugins/agent-toolkit-server.ts` — **opencode plugin entrypoint** (secondary). `config` hook registers `skills/` and `agents/`, and exposes all twenty-eight tools: `notion_get` / `notion_refresh` / `notion_status` / `notion_extract` + `swagger_get` / `swagger_refresh` / `swagger_status` / `swagger_search` / `swagger_envs` + `journal_append` / `journal_read` / `journal_search` / `journal_status` + `mysql_envs` / `mysql_status` / `mysql_tables` / `mysql_schema` / `mysql_query` + `pr_watch_start` / `pr_watch_stop` / `pr_watch_status` / `pr_event_record` / `pr_event_pending` / `pr_event_resolve` + `spec_pact_fragment` + `issue_create_from_spec` / `issue_status` + `gh_run`.
...
- `skills/spec-to-issues/SKILL.md` — SPEC → GitHub Issue sync skill. One SPEC = one epic, `# 합의 TODO` flat bullet 1 = one sub-issue. gh CLI delegated for auth / scope / write. Idempotent (marker-based dedupe). dryRun-first contract. Conducted by `rocky`; grace explicitly stops at the SPEC.
- `skills/gh-passthrough/SKILL.md` — generic `gh` runner with read / write / deny classify + dryRun guard. Conducted by `rocky`.
...
The 13 tools missing from the Claude Code surface are not yet removed from disk — code stays compilable for the opencode entrypoint until each is dropped in its own follow-up PR. Domains, in the order they should be dropped:
...
| 3 | `gh-cli` + `spec-to-issues` + `gh-passthrough` (must drop together) | `lib/gh-cli.ts` (+ test), `lib/github-issue-sync.ts` (+ test), `skills/spec-to-issues/`, `skills/gh-passthrough/`, inline `handleGhRun` / `handleIssueCreateFromSpec` / `handleIssueStatus` in `.opencode/plugins/agent-toolkit-server.ts` | `gh_run` (×1), `issue_create_from_spec` / `issue_status` (×2) | Removing `gh-cli` breaks both `spec-to-issues` and `gh-passthrough` — they ship together or none. |
