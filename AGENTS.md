...
- `skills/spec-pact/SKILL.md` — SPEC-합의 lifecycle (DRAFT / VERIFY / DRIFT-CHECK / AMEND) on top of `.agent/specs/INDEX.md` (LLM-wiki-inspired entry point) and per-page `.agent/specs/<slug>.md` (slug 모드) / `**/SPEC.md` (directory 모드). Conducted exclusively by `agents/grace.md`.
