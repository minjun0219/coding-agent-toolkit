...
1. `packages/*/src/` 에 TS 파일 추가/수정
2. `packages/*/test/*.test.ts` 에 `bun:test` 케이스 추가
3. `bun run typecheck && bun test` 로 검증
4. 통합 흐름이 바뀌면 `scripts/smoke.ts` 실행해 cache miss → hit 흐름 회귀 확인
...
- **context7** (`https://mcp.context7.com/mcp`, HTTP)
  - 외부 라이브러리(Bun runtime, TypeScript, MCP spec, opencode 등) 문서를 최신 상태로 가져올 때 사용.
...
- `dist/` 를 만들지 말 것. 패키지의 `main`/`exports` 는 `src/*.ts` 를 직접 가리킨다.
- 새 환경변수를 추가하면 `README.md` 표와 `packages/*/src/cli.ts` 두 곳을 같이 수정.
- MVP 범위 밖(database query, OAuth, multi-page, UI 등)은 추가하지 말 것.
