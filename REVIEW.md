...
- **Public contract break**: input/output shape of any of the 25 plugin tools (Claude Code surface 15: `openapi_*` / `mysql_*` / `journal_*` / `spec_pact_fragment`; opencode-only 10: `notion_*` / `pr_*`), the routing rules of `rocky` / `grace` / `mindy`, or the usage rules of the five skills (`notion-context`, `openapi-client`, `mysql-query`, `spec-pact`, `pr-review-watch`).
