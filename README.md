...
opencode 가 1차 host 지만, Claude Code 에서도 같은 toolkit 의 cache-first 표면을 쓸 수 있다. `.claude-plugin/plugin.json` + 루트 `.mcp.json` 의 `agent-toolkit` 항목이 stdio MCP 서버 (`bun run server/index.ts`) 를 노출하고, 19 개 tool (`notion_*` ×4, `openapi_*` ×5, `journal_*` ×4, `mysql_*` ×5, `spec_pact_fragment` ×1) 을 등록한다. `pr_*` / `gh_run` / `issue_*` 9 개 tool 은 추후 정리될 [REMOVAL_CANDIDATES](./REMOVAL_CANDIDATES.md) 도메인이라 Claude Code surface 에서는 빠진다 (코드는 그대로 살아 있어 opencode 진입점은 28 tool 그대로 동작).

> **Notion 인증 전제**: `notion_*` tool 은 cache miss 시 OAuth 토큰 캐시 파일 (`~/.local/share/opencode/mcp-auth.json` 또는 `AGENT_TOOLKIT_MCP_AUTH_FILE` 가 가리키는 경로) 이 있어야 remote Notion MCP 를 호출한다. 이 파일은 본래 opencode 가 OAuth 로그인 시에 만든다 — Claude Code 만 쓰는 환경이라면 (a) 한 번 opencode 에서 Notion 에 OAuth 로그인해 동일 파일을 생성하거나, (b) 동일 형식의 JSON (`{ "<key>": { "serverUrl": "...", "tokens": { "accessToken": "..." } } }`) 을 직접 두거나, (c) Notion 도구를 안 쓰면 된다 (다른 18 tool 은 영향 없음).

```bash
# 한 번만 — 저장소 root 에서
bun install

# Claude Code 가 .mcp.json 의 agent-toolkit / context7 두 서버를 처음 로드할 때
# trust prompt 가 한 번씩 뜬다 — 둘 다 승인하면 끝.
```

자세한 진입점 설계는 [`CLAUDE.md`](./CLAUDE.md), 제거 후보 목록은 [`REMOVAL_CANDIDATES.md`](./REMOVAL_CANDIDATES.md).
...
| `AGENT_TOOLKIT_MCP_AUTH_FILE` | `~/.local/share/opencode/mcp-auth.json` | Notion remote MCP OAuth 토큰 캐시 파일. opencode 가 OAuth 로그인 시 이 위치에 남기는 JSON. Claude Code 만 쓰는 사용자는 동일 형식의 파일을 직접 만들거나 이 변수로 자기 호스트의 OAuth cache 위치를 가리켜야 `notion_get` cache miss 시 remote 호출이 동작한다. |
| `AGENT_TOOLKIT_NOTION_MCP_AUTH_KEY` | `notion` | 위 auth 파일 안에서 Notion 항목을 식별하는 key. opencode 의 기본 키 이름과 동일. |
