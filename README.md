...
plugin 로드가 올바르게 되었는지 `package.json`의 entrypoint 설정을 확인합니다:

```bash
bun -e 'const p=await Bun.file("package.json").json(); console.log("main:", p.main); console.log("server:", p.exports?.["./server"]?.import)'
```

정상적인 경우 `main`은 `./.opencode/plugins/agent-toolkit.ts`, `exports["./server"]`는 `./.opencode/plugins/agent-toolkit.ts`를 가리켜야 합니다.

### Agent 미표시 시 fallback

일부 환경에서 `opencode agent list`에 `rocky` 또는 `grace`가 보이지 않는 경우, 프로젝트 로컬 디렉터리에 agent 파일을 수동으로 복사하여 노출할 수 있습니다:
...
```
(git URL install 시 `node_modules` 내 경로는 환경에 따라 다를 수 있습니다.)
