...
### GitHub 정책 및 Transport

이 프로젝트는 GitHub 데이터 접근과 조작에 대해 명확한 **Transport 분리 정책**을 따릅니다.

1.  **gh-cli**: 사용자의 `gh` CLI로 작업을 위임합니다 (`issue_*`, `gh_run`). 인증, 저장소 감지, GHE 호스팅, 권한 범위는 모두 `gh`가 관리하며, toolkit은 GitHub 토큰이나 비밀번호를 저장하거나 환경변수로 요구하지 않습니다.
2.  **external-mcp**: PR 코멘트, 답글, 리뷰, 체크 상태 등 GitHub의 실시간 상태(Source-of-Truth) 조회와 코멘트 작성은 외부 GitHub MCP 서버가 담당합니다. toolkit은 PR 코멘트용 GitHub API를 직접 호출하지 않습니다.
3.  **journal-only**: `pr_*` 도구들은 GitHub 네트워크 요청 없이 로컬 저널(`journal.jsonl`)에 이벤트 큐와 상태만 기록합니다.

#### 주요 가드레일
- **No Token Storage**: toolkit 설정이나 환경변수에는 GitHub 토큰을 두지 않습니다.
- **dryRun-first**: `issue_create_from_spec`과 `gh_run`의 쓰기 명령은 기본적으로 `dryRun: true`로 동작하여 계획만 보여줍니다. `issue_status`와 `issue_create_from_spec(dryRun: true)`는 저널 기록을 생성하지 않는 읽기 전용입니다. `gh_run`은 read/dry-run/applied 모든 호출에 journal entry를 남깁니다.
- **Restricted Commands**: `gh_run pr merge`, `repo edit|delete`, `release delete`, `workflow run|enable|disable`, `run rerun|cancel` 및 환경 변경 위험 명령(`auth`, `extension`, `config`, `alias` 등)은 보안을 위해 도구 수준에서 차단(deny)됩니다. PR 머지 실행은 toolkit의 책임 밖입니다.
- **Live QA**: 프로덕션 환경에서의 실시간 테스트는 권장하지 않으며, 테스트용 저장소에서의 사용을 권장합니다.
...
│   └── plugins/
│       ├── agent-toolkit.ts        # plugin implementation + 도구 28 개 (notion 4 + swagger 5 + journal 4 + mysql 5 + pr-watch 6 + spec-pact 1 + spec-to-issues 2 + gh-passthrough 1)
...
plugin package 는 소비자 프로젝트의 `node_modules`가 아니라 opencode cache 아래에 설치된다. git URL 로 설치한 뒤 package entrypoint 를 확인하려면 실제 cache package 디렉터리를 대상으로 검증한다:
...
정상적인 경우 `agent-toolkit root: function` 이 출력된다. local checkout 을 plugin 으로 직접 쓰는 경우에는 `PLUGIN_DIR` 를 해당 checkout 경로로 지정하면 된다.

### Agent 미표시 시 fallback

일부 환경에서 `opencode agent list`에 `rocky` 또는 `grace`가 보이지 않는 경우, 실제 plugin package 에서 프로젝트 로컬 디렉터리로 agent 파일을 수동 복사하여 노출할 수 있습니다:
...
```
(local checkout 을 plugin 으로 직접 쓰는 경우에는 `PLUGIN_DIR` 를 해당 checkout 경로로 지정한다.)
...
| 도구 | 동작 |
| --- | --- |
| `gh_run` | `args` (`gh` subcommand 부터 시작하는 문자열 배열) 를 받아 분류 (`read` / `write` / `deny`) → 정책 적용. **read** (auth status / repo view / issue list / pr view / api default GET / search / gist list\|view / ...) 는 즉시 실행. **write** (issue create / label create / api --method POST / ...) 는 `dryRun: true` (기본) 면 plan 만, `dryRun: false` 로 명시해야 실행. **deny** (`pr merge` / `repo edit\|delete` / `release delete` / `workflow run\|enable\|disable` / `run rerun\|cancel` / `auth login\|logout\|refresh\|setup-git\|token` / `extension *` / `alias *` / `config *` / `gist create\|edit\|delete\|clone` / 알 수 없는 subcommand) 는 `GhDeniedCommandError` 로 즉시 throw — `gist list\|view` 는 read 로 허용. **모든 호출(read/dry-run/applied)에 journal entry 자동 append** (tags `["gh-passthrough", "read"\|"dry-run"\|"applied"]`) |
