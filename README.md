...
opencode 가 1차 host 지만, Claude Code 에서도 같은 toolkit 의 cache-first 표면을 쓸 수 있다. `.claude-plugin/plugin.json` + 루트 `.mcp.json` 의 `agent-toolkit` 항목이 stdio MCP 서버 (`bun run server/index.ts`) 를 노출하고, 15 개 tool (`openapi_*` ×5, `journal_*` ×4, `mysql_*` ×5, `spec_pact_fragment` ×1) 을 등록한다. 노출에서 빠지는 13 개 tool 은 모두 [REMOVAL_CANDIDATES](./REMOVAL_CANDIDATES.md) 에 분류돼 있다 — `notion_*` ×4 (Claude Code 전용 인증 경로 부재), `pr_*` ×6 (pr-watch 도메인), `gh_run` ×1 (gh-passthrough), `issue_*` ×2 (spec-to-issues). 코드는 그대로 살아 있어 opencode 진입점은 28 tool 그대로 동작한다.
