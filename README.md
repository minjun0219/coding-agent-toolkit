...
1. **Plugin 등록**. 현재는 로컬 checkout 경로 등록을 권장한다. 프로젝트의 `opencode.json` 또는 사용자 단위 opencode 설정에 추가하고 opencode 를 재시작.

   ```bash
   git clone https://github.com/minjun0219/agent-toolkit.git ~/dev/agent-toolkit
   cd ~/dev/agent-toolkit && bun install
   ```
   ...
   `agent-toolkit@git+https://...` 등록은 opencode `1.14.x` 일부 환경에서 cache wrapper package 로 잡혀 실제 package entrypoint / agents 검증이 꼬일 수 있다. 자세한 검증과 fallback 은 [`.opencode/INSTALL.md`](./.opencode/INSTALL.md) 참고.
...
4. **다음 단계**. 자주 쓰는 흐름:
   ...
   - **SPEC → GitHub Issue** — `@rocky <slug> unstable_issue_status` (dryRun 먼저)
...
| PR review watch (`unstable_pr_*`, 6 개) | polling-only PR 리뷰 lifecycle (GitHub API 직접 호출 없음) | [FEATURES.md#pr-review-watch-unstable_pr_](./FEATURES.md#pr-review-watch-unstable_pr_) |
...
| spec-to-issues (`unstable_issue_*`, 2 개) | 잠긴 SPEC → GitHub epic + sub-issue 시리즈 (gh CLI 위임) | [FEATURES.md#spec-to-issues-unstable_issue_](./FEATURES.md#spec-to-issues-unstable_issue_) |
| gh-passthrough (`unstable_gh_run`, 1 개) | ad-hoc gh CLI 위임 (read / write / deny 분류 + dryRun) | [FEATURES.md#gh-passthrough-unstable_gh_run](./FEATURES.md#gh-passthrough-unstable_gh_run) |
...
`unstable_` prefix 가 붙은 도구는 아직 안정화된 agent contract 가 아니다. 현재 GitHub 계열 `unstable_pr_*`, `unstable_issue_*`, `unstable_gh_run` 은 민준이 직접 테스트해 보고 승격할 surface 로 둔다. Agent 는 이 도구들을 장기 호환 이름으로 가정하지 말고, 특히 write/dryRun 관련 작업은 plan 을 먼저 보여 주고 사람 확인을 선호해야 한다.

## GitHub Transport 정책

이 toolkit 은 GitHub 데이터 접근에 대해 명확한 **transport 분리** 를 따른다.

1. **gh-cli** — write 작업 (`unstable_issue_*`, `unstable_gh_run`) 은 사용자 `gh` CLI 에 위임. 인증 / 저장소 감지 / GHE / scope 모두 `gh` 가 관리하며, toolkit 은 GitHub 토큰을 저장하지 않고 env 변수로도 요구하지 않는다.
...
3. **journal-only** — `unstable_pr_*` 도구들은 GitHub 네트워크 요청 없이 로컬 저널 (`journal.jsonl`) 에 이벤트 큐와 상태만 기록.

주요 가드레일:
- **No Token Storage** — toolkit 설정과 환경변수에 GitHub 토큰을 두지 않는다.
- **dryRun-first** — `unstable_issue_create_from_spec` / `unstable_gh_run` 의 write 명령은 기본 `dryRun: true` 로 plan 만 보여 준다. `unstable_issue_status` 는 read-only (저널 기록 없음). `unstable_gh_run` 은 read / dry-run / applied 모든 호출에 저널 entry 를 남긴다.
- **Restricted Commands** — `unstable_gh_run pr merge`, `repo edit|delete`, `release delete`, `workflow run|enable|disable`, `run rerun|cancel`, 환경 변경 위험 명령 (`auth` / `extension` / `config` / `alias` 등) 은 도구 수준에서 deny. PR 머지 실행은 toolkit 책임 밖.
