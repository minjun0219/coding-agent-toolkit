...
- **dryRun-first**: `issue_create_from_spec`과 `gh_run`의 쓰기 명령은 기본적으로 `dryRun: true`로 동작하여 계획만 보여줍니다. `issue_status`와 dry-run 모드는 저널 기록을 생성하지 않는 읽기 전용 작업입니다.
- **Restricted Commands**: `gh_run pr merge` 및 환경 변경 위험 명령(`auth`, `extension`, `config`, `alias` 등)은 보안을 위해 도구 수준에서 차단(deny)됩니다. PR 머지 실행은 toolkit의 책임 밖입니다.
