...
2. **Toolkit skill route — follow the SKILL.md's tool-usage rules verbatim.**
   ...
   - `gh-passthrough`: `gh_run({ args: [...] })` for ad-hoc gh calls. Read commands execute immediately. Write commands default to `dryRun: true` — surface the plan and wait for explicit "apply" before re-calling with `dryRun: false`. Deny throws (auth login / extension / alias / config / gist) are not retried and not bypassed via shell tricks.
