...
1. **Extract a handle and pick a route.** *Routing decisions live in this single step — when a future sub-agent (e.g. a dedicated DB specialist) is added, only this branch list needs to point at the new agent passthrough.*
   ...
   - SPEC slug / path + a sync keyword ("이슈로 만들어" / "GitHub 이슈 동기화" / "이슈 상태" / "issue 시리즈") → `spec-to-issues` skill (Rocky conducts directly, not `@grace`). Always `unstable_issue_status` (or `dryRun: true`) first.
    - An ad-hoc GitHub keyword without a SPEC ("이슈 검색", "label 만들어줘", "PR 머지", "release 보여줘", "GitHub API 로 …", literal `gh <subcommand>`) → `gh-passthrough` skill (`unstable_gh_run` tool). Write commands always `dryRun: true` first; deny throws are not retried. **`unstable_gh_run pr merge` 는 plugin 단에서 deny 되어 있으므로 사용자에게 직접 수행을 권고.**
...
2. **Toolkit skill route — follow the SKILL.md's tool-usage rules verbatim.**
   ...
   - `spec-to-issues`: `unstable_issue_status` (or `unstable_issue_create_from_spec` with `dryRun: true`) first to surface the plan. Wait for explicit "apply" / "동기화 진행" before calling `unstable_issue_create_from_spec` with `dryRun: false`. Never bundle status + apply in one turn. `gh` precondition errors (`GhNotInstalledError` / `GhAuthError`) surface the one-line guide and stop — do not retry.
   - `gh-passthrough`: `unstable_gh_run({ args: [...] })` for ad-hoc gh calls. Read commands execute immediately. Write commands default to `dryRun: true` — surface the plan and wait for explicit "apply" before re-calling with `dryRun: false`. Deny throws (`auth login|logout|refresh|setup-git|token` / `extension *` / `alias *` / `config *` / `gist create|edit|delete|clone`) are not retried and not bypassed via shell tricks. `gist list|view` is read-allowed.
