...
- **Out of scope (grace never does directly)**:
  - **GitHub Issue 동기화 (`spec-to-issues`).** SPEC → epic + sub-issue 시리즈 sync 는 Rocky + `spec-to-issues` skill (Phase 2) 의 책임. grace 의 finalize/lock 권한은 SPEC 까지 — 한 번 잠긴 SPEC 이 GitHub 으로 어떻게 옮겨가는지는 Rocky 가 conduct.
